...
void ModuleGCS::main(){
	...
	/* make task */
	osThreadNew(heartbeatThread, (void*)&moduleGCS, NULL);
	osThreadNew(attitudeThread, (void*)&moduleGCS, NULL);
	osThreadNew(globalPositionNedThread, (void*)&moduleGCS, NULL);
...
void ModuleGCS::sendHeartbeat(){
	static uint8_t txBuffer[MAVLINK_MAX_PACKET_LEN];
	static mavlink_message_t sendMsg;
	static uint16_t len;
...
void ModuleGCS::sendAttitude(){
	static uint8_t txBuffer[MAVLINK_MAX_PACKET_LEN];
	static mavlink_message_t sendMsg;
	static uint16_t len;
	static struct BodyAngularVelocity bodyAngleVelocitySub = {0,};
	static struct Attitude attitudeSub = {0,};
...
void ModuleGCS::sendGlobalPositionNED(){
	static uint8_t txBuffer[MAVLINK_MAX_PACKET_LEN];
	static mavlink_message_t sendMsg;
	static uint16_t len;
