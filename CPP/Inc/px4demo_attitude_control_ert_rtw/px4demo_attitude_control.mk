###########################################################################
## Makefile generated for Simulink model 'px4demo_attitude_control'. 
## 
## Makefile     : px4demo_attitude_control.mk
## Generated on : Sat Jul 11 16:14:02 2020
## MATLAB Coder version: 5.0 (R2020a)
## 
## Build Info:
## 
## Final product: $(RELATIVE_PATH_TO_ANCHOR)/px4demo_attitude_control.elf
...
PRODUCT_NAME              = px4demo_attitude_control
MAKEFILE                  = px4demo_attitude_control.mk
MATLAB_ROOT               = C:/PROGRA~1/MATLAB/R2020a
MATLAB_BIN                = C:/PROGRA~1/MATLAB/R2020a/bin
MATLAB_ARCH_BIN           = $(MATLAB_BIN)/win64
MASTER_ANCHOR_DIR         = 
START_DIR                 = C:/Users/junwoo/Desktop/Second_attitude_control_worked/Second_attitude_control_worked
SOLVER                    = 
SOLVER_OBJ                = 
CLASSIC_INTERFACE         = 0
TGT_FCN_LIB               = ISO_C
...
COMPILER_COMMAND_FILE     = px4demo_attitude_control_comp.rsp
CMD_FILE                  = px4demo_attitude_control.rsp
C_STANDARD_OPTS           = 
CPP_STANDARD_OPTS         = 
...
PRODUCT = $(RELATIVE_PATH_TO_ANCHOR)/px4demo_attitude_control.elf
...
DEFINES_ = -DMODEL=px4demo_attitude_control -DNUMST=1 -DNCSTATES=0 -DHAVESTDIO -DMODEL_HAS_DYNAMICALLY_LOADED_SFCNS=0 -DCLASSIC_INTERFACE=0 -DALLOCATIONFCN=0 -DTID01EQ=0 -DTERMFCN=1 -DONESTEPFCN=1 -DMAT_FILE=0 -DMULTI_INSTANCE_CODE=0 -DINTEGER_CODE=0 -DMT=0 -DNULL=0 -D__NO_SYSTEM_INIT -DARM_MATH_CM3=1 -D__NVIC_PRIO_BITS=3 -DEXIT_FAILURE=1 -DEXTMODE_DISABLEPRINTF -DEXTMODE_DISABLETESTING -DEXTMODE_DISABLE_ARGS_PROCESSING=1 -DSTACK_SIZE=64 -D__MW_TARGET_USE_HARDWARE_RESOURCES_H__ -DRT
...
DEFINES_STANDARD = -DMODEL=px4demo_attitude_control -DNUMST=1 -DNCSTATES=0 -DHAVESTDIO -DMODEL_HAS_DYNAMICALLY_LOADED_SFCNS=0
...
OBJS = px4demo_attitude_control.o px4demo_attitude_control_data.o startup_gcc.o system_LM3S.o syscalls.o SysTickScheduler.o m3m4m4f_multitasking.o
...
%.o : $(START_DIR)/px4demo_attitude_control_ert_rtw/%.c
	$(CC) $(CFLAGS) -o "$@" "$<"


%.o : $(START_DIR)/px4demo_attitude_control_ert_rtw/%.s
	$(AS) $(ASFLAGS) -o "$@" "$<"
...
%.o : $(START_DIR)/px4demo_attitude_control_ert_rtw/%.cpp
	$(CPP) $(CPPFLAGS) -o "$@" "$<"


%.o : $(START_DIR)/px4demo_attitude_control_ert_rtw/%.cc
	$(CPP) $(CPPFLAGS) -o "$@" "$<"
...
%.o : $(START_DIR)/px4demo_attitude_control_ert_rtw/%.cxx
	$(CPP) $(CPPFLAGS) -o "$@" "$<"
...
ert_main.o : $(START_DIR)/px4demo_attitude_control_ert_rtw/ert_main.c
	$(CC) $(CFLAGS) -o "$@" "$<"


px4demo_attitude_control.o : $(START_DIR)/px4demo_attitude_control_ert_rtw/px4demo_attitude_control.c
	$(CC) $(CFLAGS) -o "$@" "$<"


px4demo_attitude_control_data.o : $(START_DIR)/px4demo_attitude_control_ert_rtw/px4demo_attitude_control_data.c
	$(CC) $(CFLAGS) -o "$@" "$<"
